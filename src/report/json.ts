...
export function toJsonReport(report: DevWrappedReport): string {
  return JSON.stringify(report, null, 2)
}

/** 计算报告文件路径（不含扩展名）：dsh-dev-wrapped-YYYY-MM-DD */
export function reportBaseName(report: DevWrappedReport): string {
  return `${FILE_PREFIX}-${localDateKey(report.generatedAt)}`
}

/** 写入 JSON 报告文件，返回文件绝对路径 */
export async function writeJsonReport(report: DevWrappedReport, outputDir: string): Promise<string> {
