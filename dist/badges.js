...
export function badgeNameKey(id) {
    return ('badge' + id
        .split('-')
...
export function badgeDescKey(id) {
    return (badgeNameKey(id) + 'Desc');
}
/** 徽章阈值常量（真实数据验证后可调） */
const THRESHOLDS = {
    /** 深夜占比 ≥ 10% */
...
export function computeBadgeLevel(id, value, earned) {
    if (!earned || !(id in LEVEL_THRESHOLDS))
        return null;
...
export function computeBadges(report) {
    const badges = [];
    ...
    let weekendCalls = 0;
    for (const d of [5, 6])
        weekendCalls += t.weekdayActivity[d] ?? 0;
    ...
    badges.push({
        id: 'rock-solid',
        earned: report.overview.totalToolCalls >= THRESHOLDS.solidMinCalls &&
...
function longestActiveStreak(daily) {
    if (daily.length === 0)
        return { days: 0, totalCalls: 0 };
