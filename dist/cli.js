...
export function parseDateArg(s, kind, startOfDay) {
 ...
 */
export function parseAdapterChoice(input) {
    const trimmed = input.trim();
    if (trimmed === '1' || trimmed === 'dsh')
        return 'dsh';
    if (trimmed === '2' || trimmed === 'claude-code')
        return 'claude-code';
    if (trimmed === '3' || trimmed === 'all')
        return 'all';
    return null;
}
/**
 ...
 */
async function promptAdapterChoice() {
    console.log('🔍 请选择数据源：');
    ...
    return new Promise((resolve) => {
        ...
        rl.once('line', (line) => finish(parseAdapterChoice(line) ?? 'dsh'));
        rl.once('close', () => finish('dsh'));
    });
}
/** 构造年度对比指标：sessions / turns / toolCalls / activeDays（tokens 双方完整时追加） */
function buildCompareMetrics(cur, prev) {
    const mk = (key, current, previous) => ({
        key,
        current,
        previous,
    ...
    if (cur.overview.tokens && prev.overview.tokens) {
        metrics.push(mk('tokens', cur.overview.tokens.input + cur.overview.tokens.output, prev.overview.tokens.input + prev.overview.tokens.output));
    }
    return metrics;
}
/** CLI 主入口；返回进程退出码 */
export async function runCli(argv) {
    ...
    try {
        ...
        if (opts.since !== undefined)
            since = parseDateArg(opts.since, '--since', true);
        if (opts.until !== undefined)
            until = parseDateArg(opts.until, '--until', false);
    }
    catch (err) {
        console.error(`✖ ${err.message}`);
        return 1;
    ...
    for (const { adapter, dataRoot } of targets) {
        ...
        try {
            ...
            if (adapterArg === 'all') {
                console.log(`ℹ️ 已跳过 ${adapterLabel}：${err.message}`);
                continue;
            }
            throw err;
        }
        if (files.length === 0) {
            ...
            console.log(adapter.id === 'claude-code'
        ...
        const headers = adapter.peekSessionHeader
            ...
            : [];
        const valid = headers.filter((h) => h !== null);
        if (valid.length > 0) {
            ...
            console.log(`📂 发现 ${mainCount} 个主会话（另有 ${subCount} 个子代理会话，默认排除），${wsCount} 个工作目录`);
        }
        else {
        ...
        for (const f of files) {
            try {
                ...
                console.error(`⚠ 跳过无法解析的会话: ${f.filePath} — ${err.message}`);
            }
        }
    }
    if (scannedFiles === 0) {
    ...
    if (top5)
        console.log(`  ${padEnd('TOP 5 工具', 12)}${top5}`);
