 ...
 */
export async function streamZstdLines(filePath, onLine, onWarn) {
 ...
 */
async function tryStreamViaCli(filePath, onLine, onWarn) {
...
async function streamViaFzstd(filePath, onLine) {
 ...
 */
export async function readFirstZstdLine(filePath) {
...
async function readFirstLineViaCli(filePath) {
    return new Promise((resolve) => {
        const child = spawn('zstd', ['-d', '-c', filePath], { stdio: ['ignore', 'pipe', 'pipe'] });
        let settled = false;
        const finish = (v) => {
...
async function readFirstLineViaFzstd(filePath) {
    try {
        const spec = 'fzstd';
        const mod = (await import(spec));
        const compressed = await fs.readFile(filePath);
        const text = new TextDecoder('utf-8', { fatal: false }).decode(mod.decompress(new Uint8Array(compressed)));
        for (const line of text.split('\n')) {
            if (line.trim().length > 0)
                return line;
        }
