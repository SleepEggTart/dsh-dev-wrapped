...
/** 工具排行 TOP8 横向条形图 */
function renderToolBars(report, lang) {
    const top = report.toolUsage.slice(0, 8);
    if (top.length === 0)
        return `<p class="empty">${t(lang, 'noToolData')}</p>`;
...
/** 24 小时活跃分布柱状图 */
function renderHourly(report, lang) {
    ...
    const bars = report.timeline.hourlyActivity
        ...
        .join('');
    const peakText = peak !== null ? t(lang, 'peakLabel', { hh: String(peak).padStart(2, '0') }) : t(lang, 'noData');
...
/** 亮点卡片 */
function renderHighlights(report, lang) {
    const { highlights } = report;
    const cards = [];
    const dash = t(lang, 'empty');
    // 最爱工具
    cards.push(`<div class="hl-card"><div class="hl-icon">🛠️</div><div class="hl-label">${t(lang, 'favoriteTool')}</div><div class="hl-value">${highlights.favoriteTool ? esc(highlights.favoriteTool) : dash}</div></div>`);
    // 最爱项目
    const ws = highlights.favoriteWorkspace;
    cards.push(`<div class="hl-card"><div class="hl-icon">📁</div><div class="hl-label">${t(lang, 'favoriteWorkspace')}</div><div class="hl-value small" title="${ws ? esc(ws) : ''}">${ws ? esc(ws) : dash}</div></div>`);
    // 最长会话
    const ls = highlights.longestSession;
    cards.push(`<div class="hl-card"><div class="hl-icon">⏱️</div><div class="hl-label">${t(lang, 'longestSession')}</div><div class="hl-value">${ls ? fmtDuration(ls.durationMs, lang) : dash}</div><div class="hl-sub">${ls ? `${fmt(ls.toolCalls)} ${t(lang, 'unitToolCalls')} · ${ls.turns} ${t(lang, 'unitTurns')}` : ''}</div></div>`);
    // 最复杂任务
    const mc = highlights.mostComplexTask;
    cards.push(`<div class="hl-card"><div class="hl-icon">🧩</div><div class="hl-label">${t(lang, 'mostComplexTask')}</div><div class="hl-value">${mc ? `${fmt(mc.totalSteps)} ${t(lang, 'unitSteps')}` : dash}</div><div class="hl-sub">${mc ? `${mc.uniqueTools.length} ${t(lang, 'unitToolsKinds')}` : ''}</div></div>`);
    return cards.join('');
}
/** 热门会话列表 */
function renderTopSessions(report, lang) {
...
function renderToolStability(report, lang) {
    ...
    const rows = top
        .map((tool) => `<tr><td class="ws" title="${esc(tool.name)}">${esc(tool.name)}</td><td class="num">${fmt(tool.errors)}</td><td class="num">${fmt(tool.calls)}</td><td class="num">${(tool.errorRate * 100).toFixed(1)}%</td></tr>`)
        .join('');
    return `
    <section class="card">
      <h2>${t(lang, 'toolStabilityTitle')}<span class="sub">${sub}</span></h2>
...
function renderPersonality(report, lang) {
    ...
    <section class="card personality-card">
      ...
      <div class="personality-row">
        ...
        <span class="p-txt">
          <b class="p-name">${esc(t(lang, personalityNameKey(p.id)))}</b>
          <small class="p-desc">${esc(t(lang, personalityDescKey(p.id)))}</small>
        </span>
      </div>
    </section>`;
}
/** 数据源分布小节（v1.2.0：--adapter all 多数据源才渲染） */
function renderSourceDist(report, lang) {
    ...
    const rows = report.adapterSources
        ...
        return `<div class="src-row"><span class="src-label">${t(lang, label)}</span><span class="src-bar"><span class="src-fill" style="width:${pct}%"></span></span><span class="src-num">${fmt(s.sessions)} · ${pct}%</span></div>`;
    })
        .join('');
    return `
    <section class="card">
...
function renderBadges(report, lang) {
...
function renderYearComparison(report, lang) {
...
export function toHtmlReport(report, lang = 'zh') {
    ...
    const dataSourceText = t(lang, 'footerDataFrom', {
        source: t(lang, report.adapterId === 'claude-code'
            ? 'dataSourceClaude'
...
export async function writeHtmlReport(report, outputDir, lang = 'zh', 
