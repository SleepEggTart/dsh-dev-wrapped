...
export function fmtDuration(ms, lang) {
    ...
    const parts = [];
    if (days > 0)
        parts.push(u(days, lang === 'zh' ? '天' : 'd'));
    if (hours > 0)
        parts.push(u(hours, lang === 'zh' ? '小时' : 'h'));
    if (mins > 0 && days === 0)
        parts.push(u(mins, lang === 'zh' ? '分钟' : 'm'));
    return parts.slice(0, 2).join(' ') || u(mins, lang === 'zh' ? '分钟' : 'm');
}
/** token 数值友好化（zh：万/亿；en：K/M） */
export function fmtTokens(n, lang) {
    if (lang === 'zh') {
        if (n >= 100_000_000)
...
export function fmtDateTime(time) {
