...
export function toStoryReport(report, lang = 'zh') {
    ...
    const dataSource = t(lang, report.adapterId === 'claude-code'
        ? 'dataSourceClaude'
    ...
    if (highlights.favoriteWorkspace) {
        ...
        <p class="tail">${wsToolCalls > 0 ? t(lang, 'storyWorkspaceCalls', { n: fmt(wsToolCalls) }) : ''}</p>
      `));
    }
    ...
    if (report.personality) {
        ...
        <div class="personality">
          ...
          <div class="p-name">${esc(t(lang, personalityNameKey(p.id)))}</div>
          <p class="p-desc">${esc(t(lang, personalityDescKey(p.id)))}</p>
        </div>
      `));
    }
    ...
    if (earnedBadges.length > 0) {
        ...
        <div class="badge-grid">
          ${earnedBadges
            .map((b) => `<div class="badge"><div class="badge-icon">${BADGE_ICONS[b.id] ?? '🏅'}${b.level ? LEVEL_ICONS[b.level] : ''}</div><div class="badge-name">${esc(t(lang, badgeNameKey(b.id)))}</div></div>`)
            .join('')}
        </div>
      `));
    }
...
export async function writeStoryReport(report, outputDir, lang = 'zh') {
