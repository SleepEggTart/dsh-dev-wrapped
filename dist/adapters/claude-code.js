...
function deriveIdentity(filePath, fallbackSessionId) {
...
export class ClaudeCodeAdapter {
     ...
     */
    async parse(file, onEvent) {
        const { sessionId, origin } = deriveIdentity(file.filePath, file.sessionId);
        // 会话头要素：首条带时间戳的行 + 首个非空 cwd（二者可能不在同一行）
        let headerCreatedAt = null;
        let headerCwd = '';
        const events = [];
        ...
        const handleUserText = (text, isMeta, ts) => {
        ...
        for await (const line of lineStream(file.filePath)) {
            ...
            if (headerCreatedAt === null && time !== null)
                headerCreatedAt = time;
            if (!headerCwd && typeof rec.cwd === 'string' && rec.cwd)
                headerCwd = rec.cwd;
            const ts = time ?? 0;
