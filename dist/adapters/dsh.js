...
export class DshAdapter {
    ...
    async peekSessionHeader(file) {
        ...
        if (!isRecord(rec) || rec.type !== 'session')
            return null;
        const id = str(rec.id);
        const cwd = str(rec.cwd);
        if (!id || !cwd)
            return null;
        const origin = rec.origin === 'subagent' || num(rec.delegationDepth) >= 1 ? 'subagent' : 'main';
    ...
    async parse(file, onEvent) {
        ...
        if (!result.ok) {
            throw new Error(`会话文件解压失败（${result.method}）: ${file.filePath}${result.error ? ' — ' + result.error : ''}`);
        }
    }
}
/**
 ...
 * 导出供单测直接验证映射逻辑。
 */
export function mapDshLine(rec, getSessionId, setSessionId, onEvent) {
    const type = rec.type;
